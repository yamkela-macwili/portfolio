...
export default function EducationEditor() {
    ...
    if (!isNew && id) {
      async function fetchEdu() {
        try {
          ...
          console.error(err);
          setSaveStatus({ type: 'error', message: 'Error loading education entry' });
        } finally {
          setLoading(false);
        }
      }
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      setSaveStatus({ type: 'success', message: 'Education saved successfully!' });
  ...
  return (
    <div className="pt-32 pb-24 px-6 max-w-2xl mx-auto min-h-screen">
      <div className="flex items-center justify-between mb-8">
        ...
        <h1 className="text-2xl font-light text-white">{isNew ? 'New Education' : 'Edit Education'}</h1>
        {!isNew && (
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-zinc-400 mb-1">Institution</label>
            <input 
          ...
          <div>
            <label className="block text-sm text-zinc-400 mb-1">Period (e.g., 2018 - 2022)</label>
            <input 
          ...
          <div>
            <label className="block text-sm text-zinc-400 mb-1">Icon (Lucide icon name, e.g., GraduationCap)</label>
            <input 
              {...register('icon')} 
              className="w-full bg-zinc-900 border border-white/10 rounded-lg p-3 text-white" 
            />
